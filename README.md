...
WOL ON LAN 是一套跨平台 Wake-on-LAN（网络唤醒）工具，包含 Android 客户端和部署在目标局域网内的代理服务。Android 客户端负责设备管理、连接测试和唤醒请求；代理服务负责在局域网中发送 UDP 魔术包。

项目提供 Windows、Linux 和 Docker 三种代理交付方式，适合家庭网络、办公室、NAS 和远程访问场景。

## 功能概览

- Android 客户端：管理设备、分组、MAC 地址、广播地址和多个代理节点。
