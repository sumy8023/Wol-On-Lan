...
```text
[2026-08-14 09:41:32][唤醒已发送]MAC=AA:BB:CC:DD:EE:FF，目标地址=192.168.1.255，端口=9，来源=203.0.113.10
```
...
如果 `address` 是普通设备 IP，例如 `192.168.1.14`，代理服务发送时会临时构造成 `192.168.1.255`，不会修改 APK 中保存的地址。
